...
macOS 浏览器测试使用已安装的 Google Chrome；Linux/CI 先运行 `npx playwright install --with-deps chromium`。测试直接通过本地 Wrangler Worker 运行，覆盖 `/api/live` 的 GET/HEAD/405、真实 404、安全响应头，以及规范目录完整性、桌面与手机、已有三舰型七视图、17 系列筛选、占位与未知参数、全屏与横竖屏布局、数据切换及滚动恢复、深链接、键盘/焦点、WCAG A/AA、生产缺图和重试。
